...
const useMutationRequest = <T>(
  ...
  showSideNotificationOnFailedError?: boolean,
  showSideNotificationOnServerError?: boolean,
  showSideNotificationOnNotFoundError?: boolean,
  showSideNotificationTooManyRequestsError?: boolean
) => {
  ...
  const _queriesToInvalidate = queriesToInvalidate
    ? [userDataRState?.id, ...queriesToInvalidate]
    : [];
    ...
    onSuccess: async (_data) => {
      if (queriesToInvalidate && _queriesToInvalidate?.length > 0) {
        await queryClient.invalidateQueries(
          _queriesToInvalidate as QueryFilters
        );
      }
    },
    onError: (err: { code?: ResponseCodeEnum } & Error) => {
      const errCode = [
        ResponseCodeEnum?.failed,
        ResponseCodeEnum?.serverError,
        ResponseCodeEnum?.notFound,
        ResponseCodeEnum?.tooManyRequests,
      ];
