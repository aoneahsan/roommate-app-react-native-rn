...
import { constellationsVal, genderVal } from "zaions-tool-kit/dist/roommate";
...
const isValidRUSelectValue = (label: string) =>
  ZOD?.object(
    {
      value: ZOD?.string()
        ?.trim()
...
const isValidStringValue = (label: string) =>
  ZOD?.string({ message: `${label} is Required.` })
    ?.trim()
...
export const userUpdateValidationSchema = ZOD.object({
  ...
  [UserFormFieldsEnum.phoneNumber]: ZOD.string()
    ...
    .startsWith("+923"),
  [UserFormFieldsEnum.withdrawOptions]: ZOD.string()
    .array()
    .nonempty("Select minimum one option"),
});
...
export const registerFormValidationSchema = ZOD.object({
  ...
  [RegisterFormFieldsEnum.password]: ZOD.string()
    .min(6, { message: "Password must contain at least 6 character(s)" })
    .max(30, {
      message: "Password must contain at least 6 character(s)",
    }),
  [RegisterFormFieldsEnum.passwordConfirmation]: ZOD.string()
    .min(6, {
      message: "Password confirmation must contain at least 6 character(s)",
    })
    .max(30, {
      message: "Password confirmation must contain at most 30 character(s)",
    }),
...
export const profileFormValidationSchema = ZOD.object({
  [FormFieldsEnum.firstName]: isValidStringValue("Name"),
  [FormFieldsEnum.lastName]: isValidStringValue("Name"),
  [FormFieldsEnum.age]: isValidStringValue("Age"),
  [FormFieldsEnum.gender]: numericEnum(genderVal),
  [FormFieldsEnum.constellation]: numericEnum(constellationsVal),
  [FormFieldsEnum.hometown]: isValidStringValue("Hometown"),
  [FormFieldsEnum.language]: isValidStringValue("Language"),
});
...
export const roomPreferenceFormValidationSchema = ZOD.object({
  ...
  [FormFieldsEnum.minBudget]: ZOD.number({
    message: "Min budget is Required.",
  }).nonnegative({ message: "Min budget mush be a non-negative number." }),
  [FormFieldsEnum.maxBudget]: ZOD.number({
    message: "Max budget is Required.",
  }).positive({ message: "Max budget mush be a positive number." }),
});
...
export const locationValidationSchema = ZOD.object({
  ...
  [FormFieldsEnum.streetAddress]: ZOD?.string({
...
export const postingListStepOneValidationSchema = ZOD.object({
  ...
  [FormFieldsEnum.buildingType]: isValidStringValue("Building type"),
  [FormFieldsEnum.location]: ZOD?.object(
    {},
...
export const postingListStepThreeValidationSchema = ZOD.object({
  ...
  [FormFieldsEnum.numOfParking]: ZOD.number({
    ...
    message: "Number of parking spots must be a non-negative number",
  }),

  [FormFieldsEnum.numOfWashroom]: ZOD.number({
    required_error: "Number of washrooms is required",
  }).nonnegative({
    message: "Number of washrooms must be a non-negative number",
  }),

  [FormFieldsEnum.minimumLease]: ZOD.number({
    required_error: "Minimum lease period is required",
  }).nonnegative({
    message: "Minimum lease period must be a non-negative number",
  }),
